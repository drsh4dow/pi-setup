...
test("captures stdout and stderr and classifies success and nonzero", () =>
	Effect.runPromise(
		Effect.gen(function* () {
...
test("retains a UTF-8-safe newest 256 KiB tail with byte counts", () =>
	Effect.runPromise(
		Effect.gen(function* () {
			const manager = new BackgroundTerminalManager();
...
test("retains exact newest output after many small writes", () =>
	Effect.runPromise(
		Effect.gen(function* () {
			const manager = new BackgroundTerminalManager();
...
test("prunes to the tracked bound without evicting running entries", () =>
	Effect.runPromise(
		Effect.gen(function* () {
			const manager = new BackgroundTerminalManager();
...
test("repeated and overlapping kills settle once", () =>
	Effect.runPromise(
		Effect.gen(function* () {
			let notifications = 0;
...
test(
	"a user kill takes ownership of stalled-pipe termination already in flight",
	{
	...
	() =>
		Effect.runPromise(
			Effect.gen(function* () {
				...
				const run = manager.start({
					command: `sh -c 'trap "echo automatic-termination-started" TERM; while true; do sleep 30; done' & exit 0`,
					title: "stalled pipes",
					cwd,
				});
...
test(
	"escalates SIGTERM and cleans the POSIX process group",
	{
		skip: process.platform === "win32",
	},
	() =>
		Effect.runPromise(
			Effect.gen(function* () {
				...
				for (
					let attempt = 0;
					attempt < 50 && !processIsGone(childPid);
					attempt++
				)
					yield* wait(20);
...
test(
	"shutdown kills a process group after its shell exits",
	{
		skip: process.platform === "win32",
	},
	() =>
...
test("shutdown kills running processes without delivering completion", () =>
	Effect.runPromise(
		Effect.gen(function* () {
			let notifications = 0;
			const manager = new BackgroundTerminalManager(() => notifications++);
...
test(
	"releases inherited pipe handles after bounded termination",
	{
		skip: process.platform === "win32",
	},
	() =>
...
test(
	"bounds settlement when descendants retain inherited pipes",
	{
		skip: process.platform === "win32",
	},
	() =>
...
test("list returns output-free metadata while detail retains output", () =>
	Effect.runPromise(
		Effect.gen(function* () {
			const manager = new BackgroundTerminalManager();
			...
			try {
				const run = manager.start({
					command: nodeCommand(
						'process.stdout.write("é"); process.stderr.write("err"); setTimeout(() => {}, 30000)',
					),
					title: "metadata",
					cwd,
				});
				...
				assert.deepEqual(manager.list(), [
					{
						...metadata,
						stdout: { totalBytes: 2, truncatedBytes: 0 },
						stderr: { totalBytes: 3, truncatedBytes: 0 },
					},
				]);
				yield* manager.kill([run.id]);
