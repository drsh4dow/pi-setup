...
/** Apply changed credentials through Pi's locked storage boundary. */
function applyCredential(ctx: ExtensionContext): boolean {
  ...
  try {
    ...
    if (
      ...
      log("credential_unchanged", { provider: PROVIDER_ID });
      return true;
    }
 ...
 * no separate login:
 *
 * - Applies changed credentials through Pi's live AuthStorage on session start
 *   so they take priority over any ANTHROPIC_API_KEY.
...
const extension = async (pi: ExtensionAPI): Promise<void> => {
  ...
  const oauth: OAuthConfig = {
      ...
      if (latestAccounts.length > 1 && callbacks.onSelect) {
        const picked = await callbacks.onSelect({
          ...
          options: latestAccounts.map((a) => ({
            id: a.source,
            label: a.source === currentSource ? `${a.label} (active)` : a.label,
          })),
        });
        if (picked) {
          chosen = latestAccounts.find((a) => a.source === picked) ?? chosen;
        }
      }
    ...
    async refreshToken(credentials: OAuthCreds): Promise<OAuthCreds> {
      ...
      if (fresh) return toOAuthCreds(fresh);
