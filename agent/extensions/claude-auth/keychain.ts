...
export function writeBackCredentials(
  ...
  if (source === "file") {
    try {
      const credPath = getCredentialsFile();
      const raw = readFileSync(credPath, "utf-8");
      if (parseCredentials(raw)?.refreshToken !== expectedRefreshToken) {
        log("writeback_skipped", { source, reason: "credentials changed" });
        return false;
      }
  ...
  if (process.platform === "darwin") {
    try {
      // Claude CLI uses the macOS username, not the service name. Resolve it
      // before the guarded read to keep the read-to-write race window small.
