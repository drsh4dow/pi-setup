...
test("live Exa answer and contents", { skip: !live }, async () => {
  ...
  const answer = await searchExa("What is the purpose of example.com?");
  assert.ok(answer.answer.length > 0);
  assert.ok(answer.sources.length > 0);

  const search = await searchExa("IANA reserved domains", { numResults: 2 });
  assert.ok(search.answer.length > 0);
  assert.ok(search.sources.length > 0);

  const [content] = await fetchExaContents(["https://example.com"]);
