...
test("temporary storage persists and evicts beyond twenty responses", async () => {
  ...
  try {
    await initializeStorage("session-a", root);
    ...
    assert.equal(getResponse("response-0"), null);
    assert.equal(getResponse("response-20")?.type, "fetch");
    ...
    await initializeStorage("session-a", root);
    assert.equal(getResponse("response-0"), null);
    assert.equal(getResponse("response-20")?.items[0].content, "content");
  } finally {
