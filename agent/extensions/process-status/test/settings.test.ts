	...
	() =>
		Effect.runPromise(
			Effect.gen(function* () {
				...
				yield* Effect.addFinalizer(() => Effect.sync(() => footer.dispose?.()));
...
test(
	"initial settings contention recovers without another write and disposal cancels recovery",
	{ concurrency: false },
	() =>
		Effect.runPromise(
			Effect.gen(function* () {
				...
				yield* Effect.addFinalizer(() => Effect.sync(live.dispose));
				let disposedChanges = 0;
				const disposed = observeAutoCompaction(
					context,
					() => disposedChanges++,
				);
				yield* Effect.addFinalizer(() => Effect.sync(disposed.dispose));
