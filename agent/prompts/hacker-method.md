...
Invoke the `hacker-method` skill at `.agents/skills/hacker-method/SKILL.md`. Initialize and check the machine-local base lab before any account-dependent work; use its stable identities and isolated browser profiles rather than asking for reusable credentials already registered there.
