...
description: "Eliminate shared state before adding coordination. Use when concurrent actors might write to the same file, branch, key, or state object."
---
...
2. **Default: eliminate the shared write target.** Ask: do these actors need one canonical object, or are they publishing independent facts? For independent facts, give each actor its own owned file, key, branch, or state directory, then combine them when reading or reporting. Two workers writing their own `lastX` field into one `state.json` is still shared mutation. `indexer-state.json` + `metrics-state.json` is not.
