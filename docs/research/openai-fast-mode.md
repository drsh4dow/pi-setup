...
- Pi's provider-request hook does not expose Codex's model catalog, so the extension must keep a conservative snapshot of Codex-supported model IDs and update it when that catalog changes. For API-key use it emits canonical `fast`; for ChatGPT-auth compatibility it emits the current catalog tier ID, `priority`. Sending public-API `fast` to the private ChatGPT Codex endpoint is not equivalent to reproducing Codex's account/catalog path.
